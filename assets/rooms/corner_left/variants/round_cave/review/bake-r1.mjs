...
export async function bakeRock(room){
 const names=['rock','ceiling','floor'];
 const meshes=names.map(n=>room.getObjectByName(n));
 const signature=await rockBakeSignature(meshes);let cached;try{cached=JSON.parse(await readFile('public/textures/unified-rock-cache.json','utf8'));}catch{}const reuse=cached?.signature===signature;
 const sources=[];
 if(!reuse)for(const name of ['Diffuse','nor_gl','arm'])sources.push(await sharp('public/textures/rock_boulder_dry_'+name+'.jpg').resize(1024,1024).removeAlpha().raw().toBuffer());
 const count=meshes.reduce((n,m)=>n+m.geometry.index.count/3,0),size=4096,columns=Math.ceil(Math.sqrt(count)),tile=Math.floor(size/columns),padding=2,span=tile-1-padding*2;
  ...
  for(let i=0;i<p.count;i+=3,face++){
   ...
   if(!reuse)for(let py=0;py<tile;py++)for(let px=0;px<tile;px++){
    ...
    for(let ch=0;ch<3;ch++){
     ...
     out[0][k+ch]=Math.max(0,Math.min(255,color*[.80,.75,.66][ch]*stain*mud));out[2][k+ch]=ch===2?0:ch===1?Math.max(190,arm):255;
    }
 ...
 if(!reuse)for(let i=0;i<3;i++)await sharp(out[i],{raw:{width:size,height:size,channels:3}}).jpeg({quality:i===1?95:92}).toFile('public/textures/unified-rock-'+['diffuse','normal','arm'][i]+'.jpg');
 const result={size,triangles:count,tile,method:'world-space triplanar baked atlas',signature};await writeFile('public/textures/unified-rock-cache.json',JSON.stringify(result));return result;
}
...
export async function rockBakeSignature(meshes){
 ...
 for(const suffix of ['Diffuse','nor_gl','arm'])hash.update(await readFile('public/textures/rock_boulder_dry_'+suffix+'.jpg'));
