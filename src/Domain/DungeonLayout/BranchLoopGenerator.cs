...
namespace LethalDungeon.Domain.Dungeons
{
    ...
    public sealed class BranchLoopResult
    {
        ...
        public BranchLoopResult(GenerationResult layout,IEnumerable<BranchLoopTrace> loops)
        { Layout=layout;Loops=loops.ToList().AsReadOnly(); }
    }
    public static class BranchLoopGenerator
    {
        ...
        public static BranchLoopResult Generate(IRandomSource random,int targetRooms=40,int loopCount=2,int maxAttempts=10000,bool requireHeightChange=true)
        {
            ...
            var links=new List<DoorConnection>();var traces=new List<BranchLoopTrace>();
            ...
            while(traces.Count<loopCount){
                ...
                while(!built && attempts<maxAttempts){
                    ...
                    int reserve=(requireHeightChange?2:0)+remainingLoops*12;
            ...
            var expanded=DungeonGenerator.Generate(catalog,"entry",targetRooms,random,maxAttempts-attempts,requireHeightChange,loopCount,"junction",Snapshot());
            attempts+=expanded.Attempts;retries+=expanded.Backtracks;
            if(!expanded.Succeeded)return Fail();
            return new BranchLoopResult(new GenerationResult(expanded.Manifest,string.Empty,attempts,retries),traces);
        }
    }
}
