...
public static class RoomCatalogJson
{
    ...
    public static ConfiguredRoomCatalog Parse(string json)
    {
        ...
        try {
            ...
            bool v2=Text(root.GetProperty("schemaVersion"))=="room-catalog-v2";
            if((!v2&&Text(root.GetProperty("schemaVersion"))!="room-catalog-v1")||root.GetProperty("metresPerUnit").GetDecimal()!=0.5m)throw new ArgumentException("Unsupported schema or units.");
            ...
            foreach(var r in input){
                ...
                var sockets=r.GetProperty("sockets").EnumerateArray().Select(s=>{Object(s,v2?new[]{"id","position","facing","size","connectionType","tangentOffsets"}:new[]{"id","position","facing","size","connectionType"});var f=Text(s.GetProperty("facing"));if(!new[]{"north","east","south","west"}.Contains(f))throw new ArgumentException("Facing.");var size=Ints(s.GetProperty("size"));if(size.Length!=2||size[0]!=4||size[1]!=6||Text(s.GetProperty("connectionType"))!="standard")throw new ArgumentException("Unsupported aperture.");return new DoorSocket(Text(s.GetProperty("id")),Point(s.GetProperty("position")),Enum.Parse<Direction>(f,true),size[0],size[1],"standard",v2?Ints(s.GetProperty("tangentOffsets")):null);}).ToArray();
