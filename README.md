...
当前进度：背包负重倍率、探险时钟与三维模块布局核心已完成Green实现。125个规则测试和14个规范/配置检查全部通过，0跳过；原始Red记录保留。CI已上传，云端运行被GitHub账号账单锁阻止；团结验证工程已准备，编辑器与真机尚未运行。
...
- [三维模块布局](src/Domain/DungeonLayout/README.md)：7种原型（含四向连接房）、种子派生1～4个大环、长支线末端寻路连接、门位对接、三维占用检查、有限回退生成与独立校验；尚未接入引擎。
